; ---------------------------------------------------------------------------
; O2om — System Tray Menu & Tooltip Manager
; ---------------------------------------------------------------------------
...
class O2omTray {
    static Setup(appInstance) {
        ...
        tray.Add(O2omLang.Get("tray_toggle_pill"), (*) => appInstance.ToggleMiniPill())
        ...
        ; Double-click default action: Show Dashboard
        tray.Default := "1&"
    }

    static UpdateTooltip(timeStr, modeStr, cycleStr) {
        try {
            tip := Format("{1} — {2} ({3}) [{4}]", O2omLang.Get("app_title"), timeStr, modeStr, cycleStr)
